...
import { Plus, Pencil, ChevronUp, Loader2 } from 'lucide-react';
...
export function AdminEventsPage() {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      if (data.isTeamEvent) {
        const minVal =
          typeof data.minMembers === 'string'
            ? parseInt(data.minMembers)
            : data.minMembers;
  ...
  const handleUpdate = async (
    ...
    try {
      ...
      // Prevent changing category / event type once teams exist.
      // Other fields remain editable.
      if (event.currentTeamCount > 0) {
        if (
          patch.category !== event.category ||
          patch.isTeamEvent !== event.isTeamEvent
        ) {
          setError(
            'Cannot change category or event type after teams have registered.'
          );
          setSaving(false);
          return;
        }
      ...
      if (patch.isTeamEvent) {
        const minVal =
          typeof patch.minMembers === 'string'
            ? parseInt(patch.minMembers)
            : patch.minMembers;
      ...
      setError(
        (err as Error).message || 'Failed to update event.'
      );
    } finally {
      setSaving(false);
    }
  ...
  const nonTechEvents = events.filter(
    (e) => e.category === 'NON_TECH'
  );
    ...
    <main className="flex flex-col gap-8 py-8 px-6 max-w-7xl mx-auto w-full">
        ...
        >
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={closeState}
              className="px-5 py-3 border border-border-default text-text-secondary font-button text-button uppercase hover:opacity-70 transition-opacity"
            >
              Cancel
            </button>
      ...
      {[
        ...
        >
          ...
          {evs.length === 0 && (
            <p className="font-body text-body text-text-muted py-4">
              None yet.
            </p>
          )}
                ...
                <div className="flex flex-wrap md:flex-nowrap items-center gap-4 p-4">
                  <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
                    ...
                    <div className="flex flex-wrap gap-3 font-micro text-micro text-text-muted uppercase tracking-widest">
                      <span>
                        {event.isTeamEvent
                          ? `Team (${event.minMembers}-${event.maxMembers} members)`
                          : 'Solo'}
                      </span>
                ...
                {isEditOpen && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {hasTeams && (
                        <span className="font-micro text-micro text-text-muted border border-dashed border-border-default px-2 py-0.5 uppercase">
                    ...
                    <div className="flex gap-3">
                      <button
                        type="button"
                        onClick={closeState}
                        className="px-5 py-3 border border-border-default text-text-secondary font-button text-button uppercase hover:opacity-70"
                      >
                        Cancel
                      </button>
...
function EventFormFields({
  ...
  setField: (
    ...
    v: EventFormData[keyof EventFormData]
  ) => void;
  isNew: boolean;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="flex flex-col gap-2 md:col-span-2">
        <label className="font-micro text-micro text-text-muted uppercase tracking-widest">
          Event Name *
        </label>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          style={{
            background: form.registrationOpen
  ...
  return (
    <main className="flex flex-col gap-8 py-8 px-6 max-w-7xl mx-auto w-full">
      <div className="border-b-2 border-primary pb-6">
        <div
          className="skeleton h-12 w-40 rounded"
          style={{
