...
function toFormState(ev: SpectrumEvent): FormState {
  return {
    ...
    rules: ev.rules.join("\n"),
    prizes: ev.prizes
      .map((p) => `${p.place}:${p.reward}`)
      .join("\n"),
    prizePool: ev.prizePool ?? "",
    registrationOpen: ev.registrationOpen,
    registrationEndsAt: ev.registrationEndsAt
      ? ev.registrationEndsAt.slice(0, 16)
      : "",
...
function buildPayload(f: FormState) {
  return {
    ...
    registrationEndsAt: f.registrationEndsAt
      ? new Date(f.registrationEndsAt).toISOString()
      : undefined,
...
function EventForm({
  ...
  return (
    ...
    >
      {showIdField && (
        <div className="col-span-2 flex flex-col gap-1 md:col-span-1">
          <label className={labelClass}>
            ID (optional, slugified from name if blank)
          </label>
      ...
      <div className="flex flex-col gap-1">
        <label className={labelClass}>
          Prize Pool (e.g. ₹5,000 / Leave blank to hide)
        </label>
      ...
      <div className="col-span-2 flex flex-col gap-1">
        <label className={labelClass}>
          Prizes (place:reward, one per line)
        </label>
...
export function EventsAdminClient({
  ...
  const [eventsState, setEventsState] =
    useState<SpectrumEvent[]>(events)

  // Sync events prop to local state
  useEffect(() => {
  ...
  async function handleCreate(
    ...
    try {
      ...
      // Filter out undefined values from payload to avoid invalid SpectrumEvent
  ...
  async function handleUpdate(
    ...
    try {
      ...
      // Filter out undefined values from payload to avoid overwriting with undefined
      const filteredPayload = Object.fromEntries(
        Object.entries(payload).filter(([, value]) => value !== undefined)
      )
  ...
  async function handleDelete(id: string) {
    ...
    // Store current events state in case we need to revert
    const prevEvents = [...eventsState]
    ...
    try {
      ...
      if (listRes.ok) {
        ...
        if (listData.events) {
          // Check if the deleted event is still in the list (meaning deletion didn't persist)
          const stillExists = listData.events.some((ev: SpectrumEvent) => ev.id === id)
          if (stillExists) {
          ...
          setEventsState(listData.events)
        }
        startTransition(() => router.refresh())
      } else {
        setError("Failed to refresh event list.")
